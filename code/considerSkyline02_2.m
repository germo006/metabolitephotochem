...
for a = 1:length(compoundList.names)
    ...
    if sum(k)>0
        ...
        % Update: 05 June 2023, NPG updating so that LOQ is using a similar
        % calculation to Brianna: LOQ = 3.3*SD(zero-point)/slope
        blank_indices = find(xdata == 0);
        meanblank = mean(ydata(blank_indices));
        sdblank = std(ydata(blank_indices), "omitmissing");
        %mean blank must be moved out of the other calc.
